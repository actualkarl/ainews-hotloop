...
# Items without an og:image get matched to a brand wordmark from
# public/images/logos/. Logos are generated by scripts/build-logos.py.

LOGO_DIR_URL = "/images/logos"

# Domain → logo slug. Match against the article's source URL hostname (with
...
DOMAIN_TO_LOGO = {
    ...
    "amazon.science":          "aws",
    "rbnz.govt.nz":            "rbnz",
    "ncsc.govt.nz":            "ncsc-nz",
    "beehive.govt.nz":         "govt-nz",
    "digital.govt.nz":         "govt-nz",
    "publicservice.govt.nz":   "govt-nz",
...
LOGO_KEYWORDS: list[tuple[re.Pattern, str]] = [
    (re.compile(r"\b(openai|chatgpt|gpt-?[0-9]|sam altman|dall[- ]?e)\b", re.I), "openai"),
    (re.compile(r"\b(anthropic|claude(?: code| sonnet| opus| haiku)?)\b", re.I), "anthropic"),
    (re.compile(r"\b(google deepmind|deepmind|gemini)\b", re.I), "deepmind"),
    (re.compile(r"\b(google ai|google research|alphabet)\b", re.I), "google"),
    (re.compile(r"\b(meta ai|llama|fair lab)\b", re.I), "meta"),
    (re.compile(r"\b(xai|x\.ai|grok|elon musk)\b", re.I), "xai"),
    (re.compile(r"\b(microsoft|azure ai|copilot|github copilot)\b", re.I), "microsoft"),
    (re.compile(r"\b(nvidia|cuda|h100|h200|gb200|nemotron)\b", re.I), "nvidia"),
    (re.compile(r"\b(mistral|le chat)\b", re.I), "mistral"),
    (re.compile(r"\b(deepseek)\b", re.I), "deepseek"),
    (re.compile(r"\b(qwen|alibaba)\b", re.I), "qwen"),
    (re.compile(r"\b(hugging ?face)\b", re.I), "huggingface"),
    (re.compile(r"\b(apple intelligence|apple ai|apple silicon)\b", re.I), "apple"),
    (re.compile(r"\b(aws|amazon web services|bedrock)\b", re.I), "aws"),
    (re.compile(r"\b(rbnz|reserve bank of new zealand)\b", re.I), "rbnz"),
    (re.compile(r"\b(ncsc[- ]nz|ncsc new zealand|gcsb)\b", re.I), "ncsc-nz"),
    (re.compile(r"\b(public service commission|psc nz|department of internal affairs|mbie|beehive)\b", re.I), "govt-nz"),
]
...
def lookup_company_logo(item: dict) -> str | None:
    """Return a /images/logos/<slug>.svg URL for an item, or None if no match.

    Domain match wins; aggregator domains forward to keyword scan over title
    and summary. Caller is responsible for only calling when image_url is
    missing.
    """
    domain = _domain_of(item.get("url", "") or "")
    if domain and domain not in AGGREGATOR_DOMAINS:
        slug = DOMAIN_TO_LOGO.get(domain)
        if slug:
            return f"{LOGO_DIR_URL}/{slug}.svg"
    haystack = " ".join(filter(None, (item.get("title"), item.get("summary"))))
    if not haystack:
        return None
    for pattern, slug in LOGO_KEYWORDS:
        if pattern.search(haystack):
            return f"{LOGO_DIR_URL}/{slug}.svg"
    return None


def assign_logo_fallbacks(candidates: list[dict]) -> int:
    """Set image_url to a company-logo URL for every candidate still missing
    an image after og:image enrichment. Returns the number assigned."""
    assigned = 0
    for item in candidates:
        if item.get("image_url"):
            continue
        logo_url = lookup_company_logo(item)
        if logo_url:
            item["image_url"] = logo_url
            item["image_source"] = "logo"
            assigned += 1
    return assigned
...
def main():
    ...
    logo_assigned = assign_logo_fallbacks(candidates)
    if logo_assigned:
        print(f"[prefetch] logo fallback: {logo_assigned} items got a brand wordmark",
              flush=True)
